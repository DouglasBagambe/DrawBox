...
contract KRNL is Ownable {
    ...
    modifier onlyAuthorized(KrnlPayload memory krnlPayload, bytes memory params) {
        if (!_isAuthorized(krnlPayload, params)) revert UnauthorizedTransaction();
        _;
    }
    ...
    function _isAuthorized(KrnlPayload memory payload, bytes memory functionParams) private view returns (bool) {
        (bytes memory kernelResponseSignature, bytes32 kernelParamObjectDigest, bytes memory signatureToken, uint256 nonce, bool finalOpinion) = 
            abi.decode(payload.auth, (bytes, bytes32, bytes, uint256, bool));

        if (!finalOpinion) revert("Final opinion reverted");
        ...
        if (recoveredAddress != tokenAuthorityPublicKey) revert("Invalid signature for kernel responses");
        ...
        if (_kernelParamsDigest != kernelParamObjectDigest) revert("Invalid kernel params digest");

        bytes32 dataDigest = keccak256(abi.encodePacked(functionParamsDigest, kernelParamObjectDigest, msg.sender, nonce, finalOpinion));
        recoveredAddress = ECDSA.recover(dataDigest, signatureToken);
        if (recoveredAddress != tokenAuthorityPublicKey) revert("Invalid signature for function call");
